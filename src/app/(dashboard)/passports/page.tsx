...
export default async function PassportsPage() {
  try {
    ...
    // If any query fails, show the empty state rather than crashing
    console.error("[PassportsPage] error:", err);
    return (
      <div className="space-y-6 max-w-6xl">
        <PageHeader
          title="Passports"
          description="0 Digital Product Passports"
          actions={
            <div className="flex items-center gap-2">
              <Link
                href="/bulk-upload"
        ...
        />
        <PassportList initialPassports={[]} brandId="" />
      </div>
    );
  }
}
