...
function buildEps(
  ...
  const lines: string[] = [
    ...
    "%%LanguageLevel: 2",
    "%%Creator: Origins Digital Product Passport",
...
// ── PDF sheet ─────────────────────────────────────────────────────────────────
// A4 branded sheet: Origins header, product name, large QR, IDs, print note.
...
async function buildPdfSheet(
  ...
  // ── Footer ────────────────────────────────────────────────────────────────
  page.drawText("Generated by Origins · origins-id.com", {
    x: pad, y: 28,
...
export async function GET(
  ...
  const targetUrl = activeQr?.target_url
    ?? (passport.slug ? `${process.env.NEXT_PUBLIC_PUBLIC_PASSPORT_URL ?? "https://passport.origins-id.com"}/${passport.slug}` : "");
